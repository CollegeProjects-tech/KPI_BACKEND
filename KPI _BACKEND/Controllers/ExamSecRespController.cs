...
namespace KPI_BACKEND.Controllers
{
    ...
    public class ExamSecRespController : Controller
    {
        private readonly IExamSecResp examSecRespRepository;

        //private readonly IMemoryCache _memoryCache;

        public ExamSecRespController(IExamSecResp examSecResprepository)
        {
            this.examSecRespRepository = examSecResprepository;
            //this._memoryCache = memoryCache;
        }

        [HttpGet("ExamSecResp")]
        public async Task<ActionResult> ExamSecResp()
        {
            var cacheKey = "MyKey";
            try
            {
                /*  var employeeList = await loginRepository.Getlogindetails();*/
                return Ok(await examSecRespRepository.ExamSecResp());
            }
            catch (Exception)
            {
        ...
        [HttpPost("SaveExamSecResp")]
        [RequestSizeLimit(3 * 1024 * 1024)] // 3MB max
        public async Task<IActionResult> SaveExamSecResp([FromForm] tbl_ExamSecResp examSecResp, IFormFile file)
        {
            try
            {
                if (examSecResp == null || file == null || file.Length == 0)
                    return BadRequest("Invalid examSecResp or file data.");
                ...
                var teacherId = examSecResp.teacher_id?.ToString().Replace(" ", "_") ?? "unknown";
                ...
                examSecResp.file_path = $"/Uploads/{teacherId}/{fileName}";



                await examSecRespRepository.SaveExamSecResp(examSecResp);

                return Ok(examSecResp);
            }
            catch (Exception)
            {
