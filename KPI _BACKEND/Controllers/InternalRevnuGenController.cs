...
namespace KPI_BACKEND.Controllers
{
    ...
    public class InternalRevnuGenController : Controller
    {
        private readonly IInternalRevnuGen internalRevnuGenRepository;

        //private readonly IMemoryCache _memoryCache;

        public InternalRevnuGenController(IInternalRevnuGen internalRevnuGenrepository)
        {
            this.internalRevnuGenRepository = internalRevnuGenrepository;
            //this._memoryCache = memoryCache;
        }

        [HttpGet("InternalRevnuGen")]
        public async Task<ActionResult> Researches()
        {
            ...
            try
            {
                ...
                return Ok(await internalRevnuGenRepository.InternalRevnuGen());
            }
            catch (Exception)
            {
        ...
        [HttpPost("SaveInternalRevnuGen")]
        [RequestSizeLimit(3 * 1024 * 1024)] // 3MB max
        public async Task<IActionResult> SaveInternalRevnuGen([FromForm] tbl_InternalRevnuGen internalRevnuGen, IFormFile file)
        {
            try
            {
                if (internalRevnuGen == null || file == null || file.Length == 0)
                    return BadRequest("Invalid internalRevnuGen or file data.");
                ...
                var teacherId = internalRevnuGen.teacher_id?.ToString().Replace(" ", "_") ?? "unknown";
                ...
                internalRevnuGen.file_path = $"/Uploads/{teacherId}/{fileName}";



                await internalRevnuGenRepository.SaveInternalRevnuGen(internalRevnuGen);

                return Ok(internalRevnuGen);
            }
            catch (Exception)
            {
