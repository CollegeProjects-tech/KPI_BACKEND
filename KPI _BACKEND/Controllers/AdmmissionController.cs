...
namespace KPI_BACKEND.Controllers
{
    ...
    public class AdmmissionController : Controller
    {
        private readonly IAdmmission admmissionRepository;

        //private readonly IMemoryCache _memoryCache;

        public AdmmissionController(IAdmmission admmissionrepository)
        {
        ...
        [HttpGet("Admmission")]
        public async Task<ActionResult> Admmission()
        {
            var cacheKey = "MyKey";
            try
            {
                /*  var employeeList = await loginRepository.Getlogindetails();*/
                return Ok(await admmissionRepository.Admmission());
            }
            catch (Exception)
            {
        ...
        [HttpPost("SaveAdmmission")]
        [RequestSizeLimit(3 * 1024 * 1024)] // 3MB max
        public async Task<IActionResult> SaveAdmmission([FromForm] tbl_Admmission admmission, IFormFile file)
        {
            try
            {
                if (admmission == null || file == null || file.Length == 0)
                    return BadRequest("Invalid admmission or file data.");
                ...
                var teacherId = admmission.teacher_id?.ToString().Replace(" ", "_") ?? "unknown";
                ...
                admmission.file_path = $"/Uploads/{teacherId}/{fileName}";



                await admmissionRepository.SaveAdmmission(admmission);

                return Ok(admmission);
            }
            catch (Exception)
            {
